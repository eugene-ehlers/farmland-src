...
def climate_for(lat: float, lon: float) -> dict:
    ...
    chirps = None
    try: chirps = monthly_chirps(lat, lon)
    except Exception: chirps = None
    monthly, rain_year = [], 0.0
    for m in range(1, 13):
        b = buckets[m]
...
def parcels(bbox: str | None = Query(default=None), limit: int = Query(default=500, ge=1, le=900)):
    if not bbox: return {"type": "FeatureCollection", "features": []}
    w, s, e, n = [float(x.strip()) for x in bbox.split(",")]
    w, s, e, n = max(w, ORIGIN_W), max(s, ORIGIN_S), min(e, ORIGIN_E), min(n, ORIGIN_N)
...
def parcel_one(parcel_id: str):
    ...
    if not parsed: raise HTTPException(404, "not a grid cell id")
...
def parcel_soil(parcel_id: str):
    ...
    if key not in SOIL_CACHE:
        try: sg = soilgrids(lat, lon)
        except Exception as exc: sg = {"has_values": False, "source": str(exc)}
        try: climate = climate_for(lat, lon)
        except Exception: climate = None
        SOIL_CACHE[key] = pack_soil(sg, climate, lat, lon)
...
def crop_one(crop_id: str):
    c = crop_card(crop_id)
    if not c: raise HTTPException(404, "unknown crop")
    return c
...
def geocode(q: str = Query(min_length=2)):
    ...
    return {"query": q, "results": [{"name": h.get("display_name"), "lat": float(h["lat"]), "lon": float(h["lon"])} for h in hits]}
